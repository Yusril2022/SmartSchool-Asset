...
<div x-data="{ open:false, collapse:false }" class="flex h-screen">
    ...
    <div x-show="open" @click="open=false"
        class="fixed inset-0 bg-black/30 z-20 md:hidden"></div>

    <!-- SIDEBAR -->
    <aside
        :class="open ? 'translate-x-0' : '-translate-x-full'"
        class="fixed md:relative z-30 h-screen transform transition-all duration-300 md:translate-x-0
        ...
        <nav class="mt-4 space-y-2 flex-1">
            ...
            <a href="{{ route('dashboard') }}"
                class="flex items-center gap-3 px-4 py-3 mx-2 rounded-lg transition
            ...
            <a href="{{ route('lokasi-ruangan.index') }}"
                class="flex items-center gap-3 px-4 py-3 mx-2 rounded-lg transition
                {{ request()->routeIs('lokasi-ruangan.*') 
                ? 'bg-orange-500 text-white shadow' 
                : 'text-gray-600 hover:bg-gray-100' }}">
            ...
            <a href="{{ route('lemari.index') }}"
                class="flex items-center gap-3 px-4 py-3 mx-2 rounded-lg transition
                {{ request()->routeIs('lemari.*') 
                ? 'bg-orange-500 text-white shadow' 
                : 'text-gray-600 hover:bg-gray-100' }}">
