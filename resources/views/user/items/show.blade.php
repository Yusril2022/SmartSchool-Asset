    ...
    <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <span class="inline-block mt-2 px-3 py-1 rounded-full text-xs font-medium
        {{ $barang->jenis_barang === 'aset' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700' }}">
                    {{ ucfirst($barang->jenis_barang) }}
                </span>
            </div>
            ...
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <p class="text-gray-800 text-sm mt-2">{{ $barang->merk }}</p>
            </div>
            @endif

            <!-- HARGA — hanya tampil untuk aset -->
            @if($barang->jenis_barang === 'aset')
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <p class="text-sm text-gray-500">Harga</p>
                <p class="text-gray-800 text-sm mt-2 font-medium">
                    Rp {{ number_format($barang->harga, 0, ',', '.') }}
                </p>
                @if($barang->is_barang_mahal)
                <p class="text-xs text-blue-500 mt-1">
                    ℹ️ Memerlukan berita acara
                </p>
                @endif
            </div>
            @endif
        ...
        <div class="mt-6">
            ...
            @else
            <a href="{{ route('item-usages.create', $barang->id) }}"
                class="block text-center bg-green-500 hover:bg-green-600 text-white py-3 rounded-xl transition shadow">
                Ambil Barang
            </a>
            @endif
